...
class Api_CalendarController extends Zend_Controller_Action
{
	...
	public function fetchAction ()
	{
		...
		if ($Authenticator->verify($sys_name, $signature)) {
			...
			$CalendarModel = new Api_Model_Calendar();
			$events = $CalendarModel->getAllEvents($sys_name);
			$return = $CalendarModel->formatEvents($events);
			$this->getResponse()
			...
			$this->getResponse()
				...
				->setHeader('Content-Type', 'text/plain', true)
				->setBody('Events inaccessible; access denied.');
		}
	}
}