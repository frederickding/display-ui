...
class Admin_ClientsController extends Admin_ControllerAbstract
{
    ...
    public function usersForm ($list = '')
    {
        ...
        array('label' => 'Accounts that should have access', 
        'required' => false, 
        ...
        array('required' => true, 'value' => $this->_getParam('id')));
        ...
        $form->setAction(
        $this->view->url(
        array('module' => 'admin', 'controller' => 'clients', 
        'action' => 'edit-users', 'id' => null)))
            ->setMethod('post')
            ->setAttrib('id', 'editusers')
            ->addElements(
        array('users' => $users, 'id' => $id, 'usercsrf' => $csrf, 
        'usersubmit' => $submit));
