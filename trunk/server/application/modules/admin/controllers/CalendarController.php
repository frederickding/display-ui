...
class Admin_CalendarController extends Admin_ControllerAbstract
{
    ...
    public function insertProcessAction ()
    {
        if (! $this->getRequest()->isPost()) {
            ...
            array('module' => 'admin', 'controller' => 'calendar', 
            'action' => 'list')));
        }
        if ($this->Acl->isAllowed($this->auth_session->userRole, 
        $this->reqController, $this->reqAction)) {
    ...
    public function insertForm ()
    {
        ...
        $this->reqController, 'insert-process')) {
            ...
            array('label' => 'Event Name', 'required' => TRUE));
            $name->setAttribs(array('cols' => 50, 'rows' => 3));
            ...
            array('label' => 'Event type', 'required' => TRUE, 
            'description' => 'Is this a one-time event or does it repeat weekly?"'));
            $type->addMultiOptions(
            array('once' => 'One time', 'weekly' => 'Weekly'));
            $time = new Zend_Form_Element_Text('eventtime', 
            array('label' => 'Event date/time', 'required' => TRUE,  // TODO: better local timezone support using CONVERT_TZ() in SQL
            ...
            'action' => 'insert-process')))
                ->setMethod('post')->addElements(
            array('eventname' => $name, 'eventclient' => $show, 
            'eventtype' => $type, 'eventtime' => $time, 'eventcsrf' => $csrf, 
            'eventsubmit' => $submit, 'eventreset' => $reset));
        }
