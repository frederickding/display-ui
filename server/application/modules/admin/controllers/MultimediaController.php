...
class Admin_MultimediaController extends Admin_ControllerAbstract
{
    ...
    public function listAction ()
    {
        ...
        $this->view->showUpload = $this->Acl->isAllowed(
        $this->auth_session->userRole, $this->reqController, 'upload');
    }
    /**
    ...
    public function uploadProcessAction ()
    {
        if (! $this->getRequest()->isPost()) {
            ...
            array('module' => 'admin', 'controller' => 'multimedia', 
            'action' => 'upload')));
        }
    ...
    public function uploadForm ()
    {
        ...
        array('label' => 'Start showing on (YYYY-MM-DD)', 'required' => false));
        $expires = new Zend_Form_Element_Text('mediumexpiration', 
        array('label' => 'Stop showing on (YYYY-MM-DD)', 'required' => false));
        ...
        $weight->addValidator('int', false)
            ->addValidator('between', false, array(1, 10))
        ...
        'action' => 'upload-process')))
            ...
            ->addElements(
        array('mediumtitle' => $title, 'mediumclients' => $clients, 
        'mediumfile' => $file, 'mediumactivatenow' => $immediateActive, 
        'mediumactivation' => $activates, 'mediumalternating' => $alternating, 
        'mediumexpiration' => $expires, 'mediumweight' => $weight, 
        'mediumduration' => $duration, 'mediasubmit' => $submit, 
        'mediareset' => $reset));
        return $form;
    }
}