...
class Api_Model_YWeather
{
    ...
    public function __construct ($location)
    {
        ...
        $this->_location = (! empty(
        $location)) ? $location : 'CAXX0504';
        Zend_Feed::registerNamespace('yweather',
    ...
    public function retrieve ()
    {
        ...
        'http://weather.yahooapis.com/forecastrss?u=c&p=' .
         $this->_location);
        return $channel;
    }
    ...
    {
        if (empty($this->_city)) {
            $this->_city = $this->retrieve()->{'yweather:location'}
                ->getDOM()
                ->getAttribute(
            'city');
        }
    ...
    public function conditions ($r)
    {
        if (! isset($this->_conditions[$r])) {
            ...
            'temp' => (int) $_data->getAttribute(
            'temp'),
            'description' => $_data->getAttribute(
            'text'),
            'code' => (int) $_data->getAttribute(
            'code'));
        }
    ...
    public function forecast ($d = 0, $param)
    {
        if (! isset($this->_forecast[0])) {
            $_data0 = $this->retrieve()
                ->current()->{'yweather:forecast'}[0]
                ->getDOM();
            $_data1 = $this->retrieve()
                ->current()->{'yweather:forecast'}[1]
                ->getDOM();
            $this->_forecast = array(
            0 => array(
            'day' => $_data0->getAttribute(
            'day'),
            'date' => $_data0->getAttribute(
            'date'),
            'high' => $_data0->getAttribute(
            'high'),
            'low' => $_data0->getAttribute(
            'low'),
            'description' => $_data0->getAttribute(
            'text'),
            'code' => $_data0->getAttribute(
            'code')),
            1 => array(
            'day' => $_data1->getAttribute(
            'day'),
            'date' => $_data1->getAttribute(
            'date'),
            'high' => $_data1->getAttribute(
            'high'),
            'low' => $_data1->getAttribute(
            'low'),
            'description' => $_data1->getAttribute(
            'text'),
            'code' => $_data1->getAttribute(
            'code')));
        }
