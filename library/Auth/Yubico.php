...
class Auth_Yubico
{
	...
	{
		if (! preg_match(
		"/^((.*)" . $delim . ")?" . "(([cbdefghijklnrtuvCBDEFGHIJKLNRTUV]{0,16})" .
	...
	function getParameters ($parameters)
	{
		...
		foreach ($parameters as $param) {
			if (! preg_match("/" . $param . "=([0-9]+)/", $this->_response,
			$out)) {
	 ...
	 * @access public
	 */
	function verify ($token, $use_timestamp = null, $wait_for_all = False, $sl = null,
	$timeout = null)
	{
		...
		if ($this->_key != "") {
			...
			hash_hmac('sha1', $parameters, $this->_key, true));
		   ...
		   */
						if (! preg_match("/otp=" . $params['otp'] . "/", $str) ||
						 ! preg_match("/nonce=" . $params['nonce'] . "/", $str)) {
							...
							hash_hmac('sha1', utf8_encode($check), $this->_key,
							true));
